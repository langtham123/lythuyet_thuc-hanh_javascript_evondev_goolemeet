window.addEventListener("load", function () {
  ...
  const todoItems = JSON.parse(localStorage.getItem(storageName) || "[]") || [];
  function init() {
    if (!todoItems || todoItems.length === 0) return;
    todoItems?.forEach((todo) => {
      renderTodo(todo);
    });
    const todoRemoveButtons = document.querySelectorAll(".todo-remove");
    todoRemoveButtons &&
  ...
  function renderTodo(todo) {
    ...
    <button class="todo-remove" data-todo="${todo}">
      <svg
  ...
  // const todoRemoveButtons = document.querySelectorAll(".todo-remove");
  // console.log("todoRemoveButtons:", todoRemoveButtons);
  // todoRemoveButtons &&
  //   todoRemoveButtons.forEach((button) => {
  //     button.addEventListener("click", removeTodo);
  //   });
...
// ["learning algorithms", "learning data structures", "learning javascript"]
const todoObj = {
  id: 1,
